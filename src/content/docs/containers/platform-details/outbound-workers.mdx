...
Use `outboundHandler` (singular) to intercept all outbound HTTP traffic regardless of destination:
...
Use `outboundHandlers` (plural) to map specific domain names or IP addresses to handler functions:
...
If you define both, domain- and IP-specific handlers in `outboundHandlers` take precedence over the catch-all `outboundHandler`.
...
Use `outboundProxies` to define named policies, then swap them at runtime using `setOutboundHandler()`. Each policy accepts a `params` object so you can configure behavior from code.
...
`wrangler dev` supports outbound interception. A sidecar process (`proxy-everything`) is spawned inside the container's network namespace. It applies `TPROXY` rules to route matching traffic to the local Workerd instance, mirroring production behavior.
